/*
 * A hwmon driver for the miniomp. 
 *
...
#define DRVNAME "miniomp_psensor"     /*Platform Sensor*/

#define SENSOR_DATA_UPDATE_INTERVAL (24*HZ)
...
static ssize_t show_psu_vout_max(struct device *dev, struct device_attribute *da,
                                 char *buf);
static int add_attr2group(struct miniomp_data *data, struct attribute *attr);
...
static struct miniomp_data *mp_data = NULL;
...
//pr_err("[%s#%d]jiffies %lu\n", __func__, __LINE__, jiffies);        
    ...
    do {
//pr_err("[%s#%d]jiffies %lu\n", __func__, __LINE__, jiffies);   
        //_tty_wait(i*TTY_RETRY_INTERVAL);
...
static int attributs_init(struct miniomp_data *data)
{
...
static void mp_data_init(struct miniomp_data *data)
{
...
//pr_err("[%s#%d]type: jiffies %lu\n", __func__, __LINE__, type, jiffies);     
    ret = bmc_transaction(cmd, resp, sizeof(resp)-1, model_ssets[type].mdelay);
    jif2 = jiffies;
    
printk("[%s#%d]type:%d Takes %lu\n",  __func__, __LINE__, type, jif2-jif);
...
update_data(struct device *dev, enum sensor_type_e type) {
    struct miniomp_data *data = mp_data;
            ...
            || !(*valid))
    {
        ...
        //pr_err("jiffies %lu(%d) for type:%d\n", jiffies, SENSOR_DATA_UPDATE_INTERVAL, type);        
...
static int miniomp_probe(struct platform_device *pdev)
{
    int status = -1;
    ...
    dev_info(&pdev->dev, "miniompbf sensors found\n");
    return 0;
...
static int miniomp_remove(struct platform_device *pdev)
{
...
static struct platform_driver miniomp_driver = {
    .probe		= miniomp_probe,
    .remove		= miniomp_remove,
    .driver		= {
        .name	= DRVNAME,
        .owner	= THIS_MODULE,
    },
...
static int __init miniomp_init(void)
{
    int ret;

    ret = platform_driver_register(&miniomp_driver);
    if (ret < 0) {
        goto exit;
    }
    mp_data = kzalloc(sizeof(struct miniomp_data), GFP_KERNEL);
    if (!mp_data) {
        ret = -ENOMEM;
        platform_driver_unregister(&miniomp_driver);
        goto exit;
    }
...
static void __exit miniomp_exit(void)
{
    if (!mp_data) {
        return;
    }
...
module_init(miniomp_init);
module_exit(miniomp_exit);
