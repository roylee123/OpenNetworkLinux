...
static int psui_create_shm(key_t id) {
    ...
    if (global_psui_st == NULL) {
        rv = onlp_shmem_create(id, sizeof(psus_status_t),
                               (void**)&global_psui_st);
        if (rv >= 0) {
            if(onlpi_create_sem(&global_psui_st->mutex) != 0) {
...
#define PMBUS_PATH_FORMAT "/sys/bus/platform/devices/miniomp_psensor/%s%d_input"
...
onlp_psui_get_string(int pid, onlp_psu_info_t* info)
{
    ...
    printf("pid:%d elapse @%lu = @%lu - @%lu\n",pid, elapse, cur, ps->last_poll);  
    if (!ps->valid || (elapse > PSU_POLL_INTERVAL)) {    
        printf("I2CRW PID:%d @%lu for PSU:%d\n", getpid(), cur, pid);  
        ...
        printf("Cached PID:%d @%lu for PSU:%d\n", getpid(), cur, pid);        
