...
static int tty_open(int *fd)
{
    ...
    do {
        ...
        i--;
        usleep(100000);
        if(0) printf("[R]Try to tty_open, %d\n", i);
    } while (i > 0);
                               ...
                               unsigned long udelay, char *buf, int buf_size)
{
    ...
    do {
        ...
        retry++ ;
        if(0) printf("[R]tty_write_and_read, retry:%d\n", retry);
    } while(ret != len && retry<TTY_RETRY && usleep(100000*retry));
...
static bool is_logged_in(int fd, char *resp, int max_size)
{
    ...
    if(0) printf("sent is_logged_in:%d\n",num);
    if (num <= 0) {
        return false;
    }
...
static int tty_transaction(const char *cmd, unsigned long udelay, char *resp, int max_size)
{
    ...
    if(0) printf("before transaction:%d\n",num);
...
uint32_t onlpi_create_sem (sem_t *mutex)
{
    int         rc;
