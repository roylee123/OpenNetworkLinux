...
#include "x86_64_accton_miniomp_log.h"
 ...
 * For miniomp, there are hot-pluggable 8 PIMs.
 * Each PIM can have 16*16Q or 4*4DD ports.
 */
#define I2C_BUS                 (1)
#define PIM_POLL_INTERVAL       (10) /*in seconds*/
#define PORT_POLL_INTERVAL      (10) /*per PIM, in seconds*/
...
static int sfpi_create_shm(key_t id) {
    ...
    if (global_sfpi_st == NULL) {
        rv = onlp_shmem_create(id, sizeof(sfpi_port_status_t),
                               (void**)&global_sfpi_st);
        if (rv >= 0) {
            if(onlpi_create_sem(&global_sfpi_st->mutex) != 0) {
...
//    printf("update_ports:  for pim:%d = %x\n", pim, present);    
    SEM_UNLOCK;
    return ONLP_STATUS_OK;
}
...
*/
static uint32_t
onlp_sfpi_reg_val_to_port_sequence(uint32_t value)
{
    int i, j;
    uint32_t ret = 0;
...
onlp_pim_is_present(int pim)
{
    ...
    if (!ps->pim.valid || (elapse > PIM_POLL_INTERVAL)) {
        ...
        if (ret < 0) {
            ps->pim.valid = false;
            ps->pim.present = 0;  /*1 means absent*/
            present = 0;
            SEM_UNLOCK;            
            return ONLP_STATUS_E_INTERNAL;
        } else {
...
get_pim_port_present_bmap(int port, uint32_t *bit_array)
{
    ...
    if(onlp_pim_is_present(pim) == 0) {
        ...
        *bit_array = onlp_sfpi_reg_val_to_port_sequence(present);
        return ONLP_STATUS_OK;
    }
    ...
    if (!ports->valid || (elapse > PORT_POLL_INTERVAL)) {
        ...
        if(port%SFP_PORT_PER_PIM==0) printf("I2CRW PID:%d @%lu for pim:%d\n", getpid(), cur, pim);
        if (ret < 0) {
            present  = 0;
            update_ports(pim, 0, present);
            *bit_array = present;        /*No needs to flip in port order.*/
            return ONLP_STATUS_E_INTERNAL;
        } else {
            present = ~present;
        ...
        if(port%SFP_PORT_PER_PIM==0) printf("Cached PID:%d @%lu for pim:%d\n", getpid(), cur, pim);    
        present = ports->present;
    }
    *bit_array = onlp_sfpi_reg_val_to_port_sequence(present);
//    printf("bit_array: %x => %x\n", pim, present, *bit_array);    
