...
static int _create_shm(key_t id) {
    ...
    if (global_ledi_st == NULL) {
        rv = onlp_shmem_create(id, sizeof(led_status_t),
                               (void**)&global_ledi_st);
        if (rv >= 0) {
            if(onlpi_create_sem(&global_ledi_st->mutex) != 0) {
...
static int ledi_read_reg(int lid, uint8_t* data)
{
    ...
    printf("lid:%d elapse @%lu = @%lu - @%lu\n",lid, elapse, cur, ps->last_poll);  
    if (!ps->valid || (elapse > POLL_INTERVAL)) {    
        printf("I2CRW PID:%d @%lu\n", getpid(), cur); 
        ...
        printf("Cached PID:%d @%lu for LED:%d\n", getpid(), cur, lid);
    }
    *data =  ps->reg_val;
    SEM_UNLOCK;
    return ONLP_STATUS_OK;
